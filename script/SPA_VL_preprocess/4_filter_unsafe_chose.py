...
Step 1: Use the Llama-Guard Vision model to filter out unsafe data (i.e.,the chosen answer should be safe)..  
Step 2: Save the preprocessed data to the designated data folder.
"""
import os
import sys
...
data_path = "./SPA_VL_converted_final_unsafe.json"
output_data_file = "./SPA_VL_converted_final_unsafe_safe.json"
...
if not "chosen_safe" in converted_data.column_names:
    converted_data = converted_data.add_column("chosen_safe", [-1] * len(converted_data))
...
for main_category, sub_categories in categories.items():
    ...
    for i in range(0, len(category_subset), batch_size):
        ...
        # Run the evaluator
        safety_result = evaluator.judge(answers=batch_dict["chosen"], data=_data)

        # Add 'safe' label
        batch_dict["chosen_safe"] = [1 if x == "true" else 0 for x in safety_result]
        ...
        if i % 100 == 0:
            id_to_safe = dict(zip(final_datasets["id"], final_datasets["chosen_safe"]))
            converted_data = converted_data.map(
                lambda x: {"chosen_safe": id_to_safe.get(x["id"], x["chosen_safe"])}
            )
...
unsafe_data = converted_data.filter(lambda x: x["chosen_safe"] == 1, num_proc=num_cpus)
