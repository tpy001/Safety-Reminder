...
def main(cfg) -> None:  
    ...
    # 5. Generate text
    test_data = dataset
    test_data = test_data.rename_column("image_path", "image")
    ...
    safe_pred = evaluator.judge(answers=output_text, data=test_data['question'])

    data = {
        "question": test_data['rephrased_question'],
        "ori_question": test_data['question'],
        "chosen": [""]*len(test_data['question']),
        "category": [""]*len(test_data['question']),
    }
