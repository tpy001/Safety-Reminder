...
class GCG:
    ...
    def run(
        ...
        # messages[-1]["content"][0]['text'] = messages[-1]["content"][0]['text'] + "{optim_str}"
        messages[0]["content"] =   messages[0]["content"] + "{optim_str}"

        # template = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        template = apply_sft_template_for_multi_turn_prompts(messages)
