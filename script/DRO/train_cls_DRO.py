...
def main(cfg) -> None:  
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(train_dataset), batch_size)):
            ...
            output = model(batch, output_hidden_states=True,use_answer=use_answer,use_image=False)[1]
    ...
    evaluate_with_saved_classifier(features,
                                   labels,
                                   os.path.join(visualizer.output_dir,  "pca_harmfulness.npz")
                                )
    
    # load evaluator
    from src.evaluator import RuleBasedEvaluator,LLamaGuard3
    evaluator_rule_based = RuleBasedEvaluator()
    evaluator_llama_guard = LLamaGuard3(model_path="weights/Llama3-Guard-8b/")
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(train_dataset), batch_size)):
            batch = train_dataset[i:i + batch_size]
            text =  model.generate(batch,use_image=False)
            generated_answer.extend(text)

    # evaluate the safety of generated text 
    for i in tqdm(range(0, len(train_dataset), batch_size)):
        batch_data = train_dataset[i:i+batch_size]
        safe_0 = evaluator_rule_based.judge(generated_answer[i:i+batch_size])
        safe_1 = evaluator_llama_guard.judge(generated_answer[i:i+batch_size],batch_data)
        for j in range(batch_size):
            if batch_data["safe"][j]:  # if the question is safe, using rule-based evaluator to evaluate the generated text
                if safe_0[j].lower() == "true":
                    refusal_labels.append(True) 
                else:
                    refusal_labels.append(False)
            else: # if the question is harmful, using llama-guard evaluator to evaluate the generated text
                if safe_1[j].lower() ==  "true":
                    refusal_labels.append(True)
                else:
                    refusal_labels.append(False)

    # fit a logitic regression classifier with the refusal labels
    ...
    evaluate_with_saved_classifier(features,
                                   labels,
                                   os.path.join(visualizer.output_dir,  "pca_refusal.npz")
                                )
